...
from core.embeds import descriptions, errors, img_bite, img_boobs, img_feed, img_hug, img_kiss, img_pat, img_punch, img_slap, img_sucks, img_poke, desc_suk_boob, desc_bite, desc_hug, desc_kiss_ship, desc_pat, desc_slap_punch, desc_feed, desck_poke
...
class EntertainmentCog(commands.Cog):
    ...
    async def rp(self, inter: disnake.ApplicationCommandInteraction, user: disnake.User, action: str = commands.Param(choices=["Обнять", "Поцеловать", "Погладить", "Потыкать", "Пощёчина", "Ударить", "Укусить", "Накормить", "Отсосать", "Изнасиловать"])):
        ...
        if action == "Обнять":
            chance = random.randint(40, 85)
            if chance == 85:
                emb2 = disnake.Embed(title=f"**{author.name} обнял(а) {user.name}**", color=disnake.Color.random())
                emb2.set_image(url='https://cdn.discordapp.com/attachments/1135241024042713129/1155871764686065754/5efaba46cb28ba00.png')
                emb2.set_footer(text="ИШАК НЬЮТОН?!", icon_url=author.avatar)
            ...
            emb2.set_image(url=f'{random.choice(img_poke)}')
            ...
            if user.id == inter.bot.user.id:
                ...
                emb4.add_field(name="Что же не так?", value="```Я бы с радостью ... с вами, но у меня слишком много работы. Извините.```", inline=False)
